...
priv_stmt='CREATE USER "mydb_slave_Emchuk"@"%" IDENTIFIED BY "emchuk"; GRANT REPLICATION SLAVE ON *.* TO "mydb_slave_Emchuk"@"%"; FLUSH PRIVILEGES;'
...
start_slave_stmt="CHANGE MASTER TO MASTER_HOST='mysql_master',MASTER_USER='mydb_slave_Emchuk',MASTER_PASSWORD='emchuk',MASTER_LOG_FILE='$CURRENT_LOG',MASTER_LOG_POS=$CURRENT_POS; START SLAVE;"
